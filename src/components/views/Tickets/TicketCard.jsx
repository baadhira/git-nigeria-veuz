    ...
    <div className="w-full max-w-[400px] relative rounded-xl overflow-visible text-white shadow-lg mx-auto">
      {badgeType && (
        <div className="absolute -top-1 -left-1 z-20 ">
          <div
            ...
            style={{
              background: badgeType === "exclusive" 
      ...
      <div className={`relative rounded-xl overflow-hidden text-white ${bgColorClass[backgroundColor]} shadow-lg h-full`}>
        <div className={`p-2 border-b border-white/20 ${bgColorClass[backgroundColor]}`}>
          ...
          <div className={`text-xs opacity-90 ml-12 text-[#E6FF00] font-bold ${badgeType ? 'ml-16' : ''}`}>
            VIEW DETAILS →
          </div>
        </div>
        ...
        <div className="relative text-white px-6 py-6 h-full">
          ...
          <div className={`relative z-10 h-full ${gitexLogo ? "py-5" : ""}`}>
            ...
            <hr className={`border-t border-gray-300 opacity-20 ${gitexLogo ? "mt-2" : "mt-2"}`} />
            
            {!isFree && (
              <div className={`flex items-center justify-between ${quantity === 0 ? "" : 'mt-2'}`}>
                <PriceDisplay 
                  price={price}
